...
const TableInstallments = ({ invoice }) => {
        ...
        <Table dir="rtl">
          <TableHeader className="text-sm">
            <TableRow className="bg-primary-50">
              ...
              <TableHead className="text-center font-semibold">
                المبلغ الرئيسي المحتفظ به
              </TableHead>
              <TableHead className="text-center font-semibold">
                المبلغ الرئيسي المكتسب
              </TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {invoice?.data?.items?.length > 0 ? (
              invoice?.data?.items.map((row, index) => (
                <TableRow key={index}>
                  ...
                  <TableCell className="text-center">
                    <SaudiCurrency value={row.installmentAmount} />
                  </TableCell>
                  <TableCell className="text-center">
                    <SaudiCurrency value={row.capitalPaid} />
                  </TableCell>
                  <TableCell className="text-center">
                    <SaudiCurrency value={row.interestPaid} />
                  </TableCell>
                  <TableCell className="text-center">
                    {formatDate(row.paidByDate) || '--'}
                  </TableCell>
                  <TableCell className="text-center">
                    <SaudiCurrency value={row.retainedPrincipal} />
                  </TableCell>
                  <TableCell className="text-center">
                    <SaudiCurrency value={row.gainedPrincipal} />
                  </TableCell>
                </TableRow>
              ))
            ) : (
