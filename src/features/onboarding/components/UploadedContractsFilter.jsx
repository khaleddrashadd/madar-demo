...
import getUploadedContractsFilter from '../services/getUploadedContractsFilter';
...
const UploadedContractsFilter = ({ isContractStatus = true, onFilter }) => {
  const {
    handleSubmit,
    reset,
    control,
  ...
  const { data: filterData, isLoading: isFilterDataLoading } = useQuery({
    queryKey: ['uploaded-contracts-filter'],
    queryFn: getUploadedContractsFilter,
    staleTime: Infinity,
  });
  ...
  return (
    <form
      ...
      className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 bg-white p-4 rounded-md"
    >
...
export default UploadedContractsFilter;
